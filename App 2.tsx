...
const App: React.FC = () => {
  ...
  const handleProcessFile = useCallback(async () => {
    ...
    try {
      ...
      // Step 1: Process files with LlamaParse
      setCurrentProcessingStage('llamaparse');
      const parseResults = await processPdfBatch(files, (progress) => {
  ...
  const handleShowInputOverview = async (frequency: DataFrequency) => {
    resetError();
    setSelectedDataFrequency(frequency);
    setIsLoadingInputs(true);
    setView(AppView.INPUT_OVERVIEW);
    try {
      const inputFolderList = await listInputFolders();
      
      // For each INPUT folder, check if there are existing OUTPUT files
      const foldersWithPreview = await Promise.all(
        inputFolderList.map(async (folder) => {
          const outputFiles = await checkExistingOutputFiles(folder.stockName);
          return {
            stockName: folder.stockName,
            inputFiles: folder.files,
            outputFiles: outputFiles
          };
        })
      );
      
      setInputFolders(foldersWithPreview);
    } catch (err) {
      handleError(err, "Could not retrieve INPUT folders from Google Drive.");
    } finally {
      ...
      case AppView.SELECTION_MENU:
        return (
          <SelectionMenu
            ...
            onProcessFromDrive={handleShowInputOverview}
          />
        );
      ...
      case AppView.INPUT_OVERVIEW:
        return (
          <InputOverview
            ...
            onSelectStock={handleSelectStockFromInput}
            onBack={handleStartOver}
            error={error}
          />
        );
      ...
      case AppView.DRIVE_FILE_SELECTION:
        return (
          <DriveFileSelection
            ...
            onBack={() => setView(AppView.INPUT_OVERVIEW)}
            error={error}
          />
        );
